...
dgi_table_filtered = dgi_table[dgi_table$interaction.score>0.7,]
...
open_genes = read.delim("/Users/ali/Downloads/gene-confidence-level.tsv", header = T)

open_genes_mechanism = read.delim("/Users/ali/Downloads/gene-aging-mechanisms.tsv")

open_genes$Genes = row.names(open_genes)

open_genes_mechanism$Genes = row.names(open_genes_mechanism)

genes_26 = read.csv("/Users/ali/Desktop/NYU/SkeletAge/26_genes.csv")


open_genes_mechanism[open_genes_mechanism$Genes %in% genes_26$Gene_Name,]

open_genes$Genes = row.names(open_genes)

skeletage_genes = readxl::read_xlsx("/Users/ali/Desktop/NYU/SkeletAge/129_important_genes_for_longevity.xlsx")

open_genes[row.names(open_genes) %in% skeletage_genes$Gene_Name,]

```

```{r}

DrugAge = read.csv("/Users/ali/Downloads/dataset/drugage.csv")

for (i in 1:nrow(dgi_table_filtered))

{

  if(nrow(DrugAge[grepl(dgi_table_filtered$drug[i], DrugAge$compound_name, ignore.case = T),]) > 0)
  {
    print(DrugAge[grepl(dgi_table_filtered$drug[i], DrugAge$compound_name, ignore.case = T),])
    print(dgi_table_filtered$drug[i])
  }
